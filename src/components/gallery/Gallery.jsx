...
function Gallery() {
  ...
  return (
    <div className="h-auto  mt-3 w-full relative flex flex-col  overflow-hidden">
      {/* First Row of Images */}
      ...
      <div className="col-span-1 sm:col-span-3 text-center">
        <h2
          ref={subtitleRef}
          className="text-[32px] sm:text-[40px] my-[11px] lg:text-[40px] font-openSans font-bold text-center"
        >
          ...
          {[Pic4, Pic5, Pic6,Pic4, Pic5, Pic6,].map((pic, index) => (
            <img
              key={`image-${index}-2`}
                           src={pic}
              alt={`pic ${index + 4}`}
              className="w-[100%] md:w-[33%] object-cover max-h-[400px] flex-shrink-0" 
                            style={{ objectPosition: "20% 20%" }} // Center
            />
          ))}
        </div>
